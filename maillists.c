...
{
	int i;
	if (data->count%PERSPACE == 0 && data->count != 0)
	{
		data->limit += PERSPACE;
		data->allMsg = (struct MailList *)realloc(data->allMsg,(data->limit + PERSPACE), sizeof(struct MailList));
	}
...
//void changeOneData(MailLists * data, int n)
//{
//	
//}
